...
import json
import base64
from config import GITHUB_FILE_PATH, GITHUB_REPO, GITHUB_TOKEN, GITHUB_FILE_PATH_SECOND_JSON
...
def commit_second_page_to_github(data):
    url = f'https://api.github.com/repos/{GITHUB_REPO}/contents/{GITHUB_FILE_PATH_SECOND_JSON}'
    headers = {
        'Authorization': f'token {GITHUB_TOKEN}',
        'Content-Type': 'application/json'
    }
    ...
    try:
        ...
        commit_data = {
            'message': f'Auto-Updated second page job data {datetime.now().isoformat()}\n{content}',
            'content': content,
            'sha': sha
        }
